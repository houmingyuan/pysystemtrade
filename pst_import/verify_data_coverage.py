...
Usage:
    python pst_import/verify_data_coverage.py
...
def check_instrument_coverage(
    ...
    try:
        ...
        first_prices = dict_of_prices[contracts[0]]
        last_prices = dict_of_prices[contracts[-1]]
        contract_start = first_prices.index.min()
        contract_end = last_prices.index.max()
    ...
    try:
        ...
        if gap_years > ACCEPTABLE_START_GAP_YEARS:
            issues.append(f"Roll calendar starts {gap_years:.1f} years after contracts")
    ...
    return CoverageResult(
        ...
        status=status,
        contract_start=contract_start.strftime("%Y-%m-%d") if contract_start else "",
        contract_end=contract_end.strftime("%Y-%m-%d") if contract_end else "",
        adj_start=adj_start.strftime("%Y-%m-%d") if adj_start else "",
        adj_end=adj_end.strftime("%Y-%m-%d") if adj_end else "",
        roll_cal_start=roll_cal_start.strftime("%Y-%m-%d") if roll_cal_start else "",
        roll_cal_end=roll_cal_end.strftime("%Y-%m-%d") if roll_cal_end else "",
        gap_days=gap_days if gap_days else 0,
        issues=issues
    )
...
def main():
    ...
    if gap_count > 0 or error_count > 0:
        print("\nTo fix gaps, try:")
        print("  python pst_import/fix_roll_calendars.py --fix")
